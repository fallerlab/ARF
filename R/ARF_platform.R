...
#' @description This function allows you to read the tab-separated ARF-samples file, returning a dataframe.
#' @param samplesFile Sample data file that describes file locations and sample groupings. A tab-separated TSV
#'   with at minimum the columns: \code{sampleName} (unique identifier), \code{bedGraphFile} (path to bedGraph
#'   or BAM alignment file), and \code{group} (condition/treatment label). Optional columns include
#'   \code{taggedRP} and \code{ribosomeIsolation}.
#' @return A data.frame with one row per sample. Columns correspond to those in the TSV. Row names are set
#'   to the sample names (first column).
#' @seealso \code{\link{dripARF}}, \code{\link{dricARF}}
...
#' Organism Check
#' @description Check if the organism code is valid for built-in ARF analysis. Valid codes are
#'   \code{'hs'} (Homo sapiens, human), \code{'mm'} (Mus musculus, mouse), and \code{'sc'}
#'   (Saccharomyces cerevisiae, yeast). When an invalid code is supplied, the function prints a
#'   message listing these options and also displays available PDB structure IDs grouped by species,
#'   which can be used as input to \code{ARF_parse_PDB_ribosome()} for custom organism analyses.
#' @param organism Organism abbreviation. Pass \code{"hs"} for human, \code{"mm"} for mouse, and
#'   \code{"sc"} for yeast.
#' @return Logical. \code{TRUE} if the organism code is valid (\code{'hs'}, \code{'mm'}, or
#'   \code{'sc'}). \code{FALSE} otherwise, printing a message that lists supported organisms and
#'   available PDB structures grouped by species.
...
ARF_check_organism <- function(organism) {
  if (!(organism %in% c("hs","mm","sc"))) {
    ...
    message("Choose a valid organism. hs (Homo sapiens, human), mm (Mus musculus, mouse), sc (Sacchramyces cerivisae)
...
#' @description Downloads a CIF structure file from RCSB for the given PDB ID (or uses a supplied CIF
#'   file), extracts rRNA N1 atom coordinates and ribosomal protein (RP) alpha-carbon coordinates, and
#'   computes the minimum Euclidean distance (in Angstroms) from each rRNA position to each RP. Returns
#'   a proximity matrix and writes two CSV output files.
#' @param species ID for the species to be analyzed, e.g. \code{"human"} or \code{"hs"}.
#' @param PDBid ID for the 3D structure from RCSB, e.g. \code{"4V6X"} for human.
#' @param PDB_file Path to a locally supplied CIF file for the 3D ribosome (optional; downloaded
#'   automatically if not provided).
#' @param rRNAs_file Path to a rRNA FASTA file previously generated by ARF for this structure
#'   (optional; downloaded automatically from RCSB if not provided).
#' @param PDB_chains_2_RP_nomenclature Alternative data.frame mapping chain IDs to RP names and new
#'   nomenclature. Required columns: \code{ID}, \code{RP_name}, \code{RP_new}, \code{chainID}.
#'   If \code{NULL}, the built-in ARF nomenclature map is used for supported PDB IDs.
#' @param download_directory Directory for downloaded CIF and FASTA files (default: \code{getwd()}).
#' @param method Analysis method to use. Default: \code{"dripARF_minDistances"}, which calculates
#'   minimum rRNA-to-RP Euclidean distances. No other options are currently implemented.
#' @param out_prefix Prefix for the output CSV filenames (default: \code{"Ribosome.3D"}).
#' @param duplication_resolve Function used to resolve duplicated chains (default: \code{min},
#'   i.e. take the minimum distance across duplicate chains).
#' @return A data.frame (\code{RP_proximity_df}) with rows = rRNA positions (row names formatted as
#'   \code{rRNA_resno}) and columns = ribosomal protein names, containing minimum Euclidean distances
#'   (Angstroms) from each rRNA N1 atom to each RP alpha-carbon. Two CSV files are also written:
#'   \code{<out_prefix>.<PDBid>.ARF.minimum_distances.csv} (long format) and
#'   \code{<out_prefix>.<PDBid>.ARF.minDist_matrix.csv} (wide matrix).
#' @seealso \code{\link{ARF_convert_Ribo3D_pos}}, \code{\link{dripARF_get_RP_proximity_sets}}
#' @keywords PDB ribosome 3D distances
#' @export
#' @examples
#' \dontrun{
...
#' @description Aligns rRNAs from the source organism (used to generate the distance file) and a target
#'   organism, then lifts over source-species 3D ribosome data to target-organism rRNA coordinates via
#'   pairwise ClustalW alignment. Short gaps (< 5 positions) in the target coordinates are filled by
#'   linear interpolation of adjacent distances.
#' @param source_distance_file Path to the \code{XXX.ARF.minimum_distances.csv} file generated by
#'   \code{ARF_parse_PDB_ribosome()} for the source 3D ribosome.
#' @param source_rRNAs_fasta FASTA file containing the rRNA sequences of the source organism.
#' @param target_species ID string for the target species, e.g. \code{"mm"}, \code{"sc"}, \code{"dm"}.
#' @param target_rRNAs_fasta FASTA file for the rRNAs of the target organism. Should be the same file
#'   used in rRNA fragment alignment.
#' @param rRNA_pairs List of 2-element character vectors matching source rRNA IDs to target rRNA IDs
#'   (necessary for lift-over). Example:
#'   \code{list(c("rRNA_28S", "dm_28S"), c("rRNA_18S", "dm_18S"))}. Pairs for rRNAs with identical
#'   names in source and target are added automatically.
#' @param source_positions Named list of position vectors to convert (alternative input when
#'   \code{type='positionset'}).
#' @param source_sets GSEA sets data.frame to convert, with \code{"ont"} and \code{"gene"} columns
#'   (alternative input).
#' @param type Which output to return. Currently \code{'distances'} (returns wide
#'   \code{RP_proximity_df}) or \code{'positionset'} (returns a named list of target position vectors).
#' @return If \code{type='distances'}: a wide data.frame (\code{RP_proximity_df}) equivalent to that
#'   from \code{ARF_parse_PDB_ribosome()}, with coordinates lifted over to the target organism. Also
#'   writes two CSV files analogous to those from \code{ARF_parse_PDB_ribosome()}. If
#'   \code{type='positionset'}: a named list of target rRNA position vectors (character, formatted as
#'   \code{rRNA_resno}).
#' @seealso \code{\link{ARF_parse_PDB_ribosome}}, \code{\link{dripARF_get_RP_proximity_sets}}
#' @keywords 3D rRNA coordinates conversion liftover
#' @export
#' @examples
#' \dontrun{
#' ARF_convert_Ribo3D_pos("Ribosome.3D.4V6X.ARF.minimum_distances.csv", "4V6X.rRNAs.fasta", "dm",
#'   "drosophila_rRNAs.fa", rRNA_pairs=list(c("rRNA_28S","dm_rRNA_28S"),
#'   c("rRNA_18S","dm_rRNA_18S"), c("rRNA_5.8S","dm_rRNA_5.8S"), c("rRNA_5S","dm_rRNA_5S")))
#' }
