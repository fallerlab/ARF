...
ARF links position-specific rRNA fragment abundances to the 3D structure of the ribosome to identify biological drivers of ribosomal heterogeneity and collision dynamics across conditions. It currently includes three methods:
...
- **driftARF** — **D**ynamic **RI**bosomal **F**eature **T**racking: the continuous-feature analogue of dripARF/dricARF. Instead of contrasting discrete groups, it correlates a numeric per-sample feature (a column in the samples file) with position-specific rRNA fragment abundances and runs the same RPSEA/ORA engine to nominate RP heterogeneity sets, ribosome collision sets, or user-given sets whose fragmentation tracks the feature.

All three methods share one engine: DESeq2 normalisation, GSEA-based enrichment testing (RPSEA), and overrepresentation analysis (ORA), with enrichment scores z-scored against 99 circularly-shifted random controls to remove positional rRNA fragmentation bias, plus an ssGSEA/limma weighting layer (ssRPSEA) producing `weighted.RPSEA.NES_randZ`. dripARF and dricARF rank rRNA positions by a DESeq2 group contrast; driftARF ranks them by Pearson correlation with a continuous sample feature.
...
driftARF_results <- ARF::driftARF(
  samplesFile = "samples.tsv",
  rRNAs_fasta = "rRNAs/mouse_rRNAs.fa",
  features    = c("pseudotime"),   # one or more numeric columns
  organism    = "mm",
  targetDir   = "driftARF_results/"
)
```

driftARF loops over each column named in `features`, correlates it with per-position rRNA fragment abundance (Pearson `r`/`p` on the variance-stabilised matrix), and writes one results CSV per feature. Useful options:
...
driftARF reuses the dripARF front end and RPSEA/ORA engine, swapping the per-position ranking statistic for a feature correlation:

| Function | Purpose |
|---|---|
| `driftARF()` | Entry point: correlate continuous feature(s) with per-position rRNA fragment abundance and run RPSEA/ORA per feature |
| `driftARF_predict_progression()` | Core: per-feature Pearson ranking → RPSEA (with random-set z-scoring) + ORA + ssRPSEA weighting |
